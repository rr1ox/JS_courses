...
// divBlock.innerText = 'bla bla bla';
// document.body.appendChild(divBlock);
// const divClone = divBlock.cloneNode(true);
// divClone.style.background = 'pink';
...
//
//     divS.classList.add('member');
//     h2.innerText = `Name: ${simpson.name} || Surname: ${simpson.surname}`;
//     age.innerText = `Age: ${simpson.age}`;
//     inf.innerText = `Info: ${simpson.info}`;
//     img.setAttribute('src', simpson.photo);
//     img.setAttribute('alt', simpson.name);
//
//     divS.append(h2, age,inf, img);
...
for (const coursesEl of coursesArray) {
    const divMain = document.createElement('div');
    const title = document.createElement('h2');

    const durationBox = document.createElement('div')
    const timeMonth = document.createElement('div');
    const timeHours = document.createElement('div');

    const ul = document.createElement('ul');

    title.innerText = coursesEl.title;
    timeMonth.innerText = coursesEl.monthDuration;
    timeHours.innerText = coursesEl.hourDuration;

    for (let i = 0; i < coursesEl.modules.length; i++) {
        const li = document.createElement('li');
        li.innerText = coursesEl.modules[i];
        ul.appendChild(li);
    ...
    ul.classList.add('ul');
    timeMonth.classList.add('timeM');
    timeHours.classList.add('timeH');

    durationBox.append(timeMonth, timeHours);
    divMain.append(title, durationBox, ul);
